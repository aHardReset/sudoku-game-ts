...
function generateNewBoard(): number[][]{
  ...
  for (let emptiesIdx = 0; emptiesIdx<randomCells.length; emptiesIdx++){
    board[Math.floor(randomCells[emptiesIdx] / side)][randomCells[emptiesIdx] % side] = emptyCellIdentifier
  }
...
function isAValidGame(currentBoard: number[][]): boolean {
  ...
  for (let row = 0; row < currentBoard.length; row++) {
    for (let col = 0; col < currentBoard[0].length; col++) {
      if (currentBoard[row][col] !== -1 && !isValid(currentBoard, currentBoard[row][col], row, col)) {
        return false
      }
    }
  }
...
function isASolvedBoard(board: number[][]): boolean{
  for (let row = 0; row < board.length; row++) {
    for (let col = 0; col < board[0].length; col++) {
      if (board[row][col] === -1 || !isValid(board, board[row][col], row, col)) {
        return false
      }
    }
  }
...
export {generateNewBoard, isValid, emptyCellIdentifier, solve, isASolvedBoard, findNextEmpty}
export type {Step}